...
class Mysql_pdo {
 ...
 public static $pconnect = true; // 是否長连接
 ...
 public function connect() {
  try {
    if(!isset(self::$DB)){
        self::$DB = new PDO ( self::$dbtype . ':host=' . self::$dbhost . ';port=' . self::$dbport . ';dbname=' . self::$dbname, self::$dbuser, self::$dbpass, array (PDO::ATTR_PERSISTENT => self::$pconnect) );
    }
 ...
 private function getCode($table, $args) {
  ...
  if (is_array ( $args )) {
   foreach ( $args as $k => $v ) {
    ...
    $code .= "`" . addslashes($k)."`='" . addslashes($v) . "',";
   }
  }
